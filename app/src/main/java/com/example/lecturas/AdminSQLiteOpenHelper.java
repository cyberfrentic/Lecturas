...
public class AdminSQLiteOpenHelper extends SQLiteOpenHelper{
    ...
    public void onCreate(SQLiteDatabase BaseDeDatos) {
        ...
        // cedulas capturadas
        BaseDeDatos.execSQL("CREATE TABLE cedula(contrato int primary key, gennombre text, gendireccion text, gencruzamientos text, gencolonia text, genmanzana text, genlote text, genopciones text, genotros text, tarttarifa text, tarvfopciones text, tartuso text, tomismedidor text, tommedidor text, tomisfunc text, tommisdesc text, tommisdesconectado text, tommisrob text, tommisina text, tommiscanc text, tomtisdirecta text, tomtiscancelada text, tomtisconolot text, tomtisclandes text, tomdmiscance text, tomdtiscence text, tomdiscotrlot text, tomdisclandes text, fuga text, imagefilename text, latitud real, longitud real, fecha text )");
